...
def indicate_word(message):
	"""Create a dictionary of entries{word: True} for every unique word in message.
	"""
...
def label_features(filelist, label):
	...
	for fl in filelist:
		tokens = []
		for word  in open(fl).read().split():
